...
function scoreJobFit(job) {
  ...
  // Salary indicators — $150K minimum target
  if (job.salary) {
    const match = job.salary.match(/\$(\d[\d,]*)/);
    if (match) {
      const salary = parseInt(match[1].replace(/,/g, ''));
      if (salary >= 200000) score += 15;
      else if (salary >= 150000) score += 10;
      else if (salary >= 120000) score += 3;
      else if (salary < 100000) score -= 15; // Too low for VP
    } else {
      score += 3; // Salary listed but unparseable — slight bonus
    }
  }
